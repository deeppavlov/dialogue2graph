...
evaluator = HuggingFaceCrossEncoder(
    model_name=env_settings.RERANKER_MODEL, model_kwargs={"device": env_settings.DEVICE}
)
embedding = {}
...
def if_greetings(node1: str, node2: str):
    ...
    if (re.search(greetings, node1) is None) is not (
        re.search(greetings, node2) is None
    ):
        greetings_cond = True
        print("GREETINGS")
    else:
