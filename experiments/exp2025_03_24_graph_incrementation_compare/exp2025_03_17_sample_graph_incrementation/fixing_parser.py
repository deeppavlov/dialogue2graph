...
from typing import Annotated, Any, TypeVar, Union, List, Callable
...
ParserHelperType = List[Callable[[str], str]]
...
class EnhancedOutputFixingParser(BaseOutputParser[T]):
    ...
    parser_helpers: ParserHelperType = None

    def __init__(self, *args, parser_helpers: ParserHelperType = None, **kwargs):
    ...
    def from_llm(
        ...
        return cls(parser=parser, retry_chain=chain, max_retries=max_retries, parser_helpers = parser_helpers)


    def parse(self, completion: str) -> T:
        retries = 0

        if self.parser_helpers:
            for helper in self.parser_helpers:
