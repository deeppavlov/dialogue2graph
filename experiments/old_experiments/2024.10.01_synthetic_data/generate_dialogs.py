...
def generate_dialog_graph_pair():
...
def main():
    ...
    for i in tqdm(range(100)):
        pair = None
        while pair is None:
            pair = generate_dialog_graph_pair()
            if pair is not None:
                pair["id"] = i + 1
                results.append(pair)
            else:
    ...
    with open(
        "experiments/2024.10.01_synthetic_data/generated_data/dialog_graph_pairs.json",
        "w",
    ) as f:
        json.dump(results, f, indent=2)

    print("Generation complete. Results saved to dialog_graph_pairs.json")
