...
def _get_tails(dialog: Dialog, utt: str) -> list[list[dict[str, str]]]:
...
def _compare_tails(dialogs: list[Dialog], utt1: str, utt2: str) -> bool:
    """Compares tails of two utterances in a list of dialogs
    Args:
...
def group_nodes(dialogs: list[Dialog], nodes_list: list[str]) -> list[list[str]]:
    """Rule based algorithm to group graph nodes based on context matching
    Args:
      dialogs: list of dialogs to form nodes from
    ...
    for idx1, assist_utt1 in enumerate(nodes_list):
        ...
        for idx2, assist_utt2 in zip(
            ...
            if _compare_tails(dialogs, assist_utt1, assist_utt2):
