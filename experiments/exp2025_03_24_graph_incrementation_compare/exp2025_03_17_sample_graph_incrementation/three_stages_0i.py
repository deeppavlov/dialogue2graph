...
class ThreeStagesGraphGenerator():
    ...
    def __init__(self, prompt_name: str=""):
        ...
        self.base_model = ChatOpenAI(model=env_settings.GENERATION_MODEL_NAME, api_key=env_settings.OPENAI_API_KEY, base_url=env_settings.OPENAI_BASE_URL)
        ...
        self.output_parser = OutputFixingParser.from_llm(self.parser_model, PydanticOutputParser(pydantic_object=DialogueGraph))
    ...
    def invoke(self, dialogue: list[Dialogue] = None, graph: Graph = None, topic: str = "") -> BaseGraph:
        ...
        graph_dict = call_llm_api(prompt.format(graph=graph.graph_dict), self.model).model_dump()
        ...
        try:
            ...
            result = call_llm_api(prompt.format(graph_dict=graph_dict), self.model)
            if result is None:
