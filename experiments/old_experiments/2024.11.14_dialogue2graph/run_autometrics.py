...
def run_all_algorithms():
    ...
    for class_ in algorithms:
        ...
        if (
            ...
            algorithms[class_]["input_type"] in [Dialogue, list[Dialogue]]
            and algorithms[class_]["output_type"] is BaseGraph
        ):
            # tp = algorithms[class_]["type"]
            ...
            if not test_list:
                for case in dialogue_to_graph:
                    ...
                    if algorithms[class_]["input_type"] is Dialogue:
                        for test_dialogue in [
                            Dialogue.from_list(c["messages"]) for c in case["dialogues"]
                        ]:
                        ...
                        result_graph = class_instance.invoke(
                            [
                                Dialogue.from_list(c["messages"])
                                for c in case["dialogues"]
                            ]
                        )
