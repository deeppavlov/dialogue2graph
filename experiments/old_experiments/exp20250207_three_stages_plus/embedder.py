...
greetings_re = re.compile(
    r"\b(let\'s|let us|let me|introduce|welcome|good morning|good evening|good afternoon|hi|hello|hey|thank|thanks|no problem|great|awesome|excellent|perfect|glad|nice|fantastic|wonderful)\b",
    re.IGNORECASE,
)
else_re = re.compile(
    r"\b(else|change|exchange|further|add|increase|increased|another|more|other|extra|additional|alternative|alternatively)\b",
    re.IGNORECASE,
)
yn_re = re.compile(
    r"\b(preferred|prefer|where|what|when|where|who|whom|which|whose|why|how)\b",
    re.IGNORECASE,
)
...
evaluator = HuggingFaceCrossEncoder(
    model_name=env_settings.RERANKER_MODEL,
...
def ends_match(node1: str, node2: str) -> tuple[bool, list[str], list[str]]:
    ...
    return (
        re.match("^.*(?<![!?])$", node1)
        and re.match("^.*(?<![!?])$", node2)
        or all([end1, end2, end1[-1] == end2[-1]]),
        sents1,
        sents2,
    )
...
def if_greetings(node1: str, node2: str):
    ...
    if (re.search(greetings_re, node1) is None) is not (
        re.search(greetings_re, node2) is None
    ):
        greetings_cond = True
        print("GREETINGS")
    else:
...
def nodes2groups(
    nodes_list: list[str], next_list: list[str], mix_list: list[str], neigbhours: dict
):
    ...
    nodes_score = get_reranking(
        nodes_list, nodes_list
    )  # Scoring of assistant utterances similarity between each other
    next_score = get_reranking(
        next_list, next_list
    )  # Scoring of user utterances following each assistant utterance
    mix_score = get_reranking(
        mix_list, mix_list
    )  # Scoring of concatenations of assistant and user together
    pairs = []
    ...
    for ind1, node1 in enumerate(nodes_list):
        ...
        for ind2, node2 in zip(
            ...
            max_n = max(
                nodes_score[ind1][ind2], nodes_score[ind2][ind1]
            )  # Maximum in pair of pairs (node1,node2) and (node2,node1) similarities
            max_m = max(
                mix_score[ind1][ind2], mix_score[ind2][ind1]
            )  # Maximum in pair of cross encoding pairs (mix1,mix2) and (mix2,mix1) similarities
            min_e = min(
                next_score[ind1][ind2], next_score[ind2][ind1]
            )  # Minimum in pair of pairs (next1,next2) and (next2,next1) similarities
            max_e = max(
                next_score[ind1][ind2], next_score[ind2][ind1]
            )  # Maximum in pair of pairs (next1,next2) and (next2,next1) similarities
            ABS = 0.99
            absolute = max_n >= ABS and max_m >= ABS and min_e >= ABS
            print(
                f"MIX: max_n:{max_n},max_m:{max_m},min_e:{min_e},max_e:{max_e}",
                nodes_list[ind1],
                nodes_list[ind2],
            )
            if (
                max_n > 0.999
            ):  # When nodes are close enough, symmetric difference is ignored
                one_word = False
            else:
            ...
            condition = absolute or not (
                greetings_cond
                or if_cond
                or else_cond
                or yes_no
                or one_word
                or (len1 == 1 and len2 == 1 and not signs)
                or node1 in neigbhours[node2]
            )
            ...
            cond_1 = (
                ...
                and min_e >= 0.06
                and max_e > 0.9
                and max_m >= env_settings.NEXT_RERANKER_THRESHOLD
                and max_n > 0.05
                or len1 != len2
                and max_n > 0.996
            )
            ...
            if condition:
                if cond_1:  # This is enough to pair nodes
                    ...
                    sent_score = []
                    if (
                        len1 > 1 and len1 == len2
                    ):  # Checks whether nodes have same number of sentences > 1
                        for s1, s2 in zip(sents1, sents2):
                            sent_score.append((s1, s2))
                            sent_score.append((s2, s1))
                        sent_score = evaluator.score(
                            sent_score
                        )  # Scoring every pair of sentences between two nodes
                        # maxes = [max(el[0],el[1]) for el in zip(sent_score[::2],sent_score[1::2])]
                        nodes_condition = (
                            max(sent_score) >= 0.9
                            and min(sent_score) >= 0.05
                            and max_n >= env_settings.RERANKER_THRESHOLD
                            and min_e >= 0.06
                            and max_e > 0.9
                        )
                    else:
                        nodes_condition = (
                            len1 == len2 and max_n >= env_settings.RERANKER_THRESHOLD
                        )
                    # print("NODCOND: ", nodes_condition)
                    if (
                        max_m >= env_settings.NEXT_RERANKER_THRESHOLD
                        and nodes_condition
                    ):
...
def get_2_rerankings(
    generated1: list[str], golden1: list[str], generated2: list[str], golden2: list[str]
):
    sz = len(generated1)
    to_score = []
