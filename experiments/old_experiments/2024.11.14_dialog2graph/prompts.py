...
create_graph_prompt = PromptTemplate.from_template(
    "You have an example of dialog from customer chatbot system. You also have an "
    ...
    "I will give a dialog, your task is to build a graph for this dialog in the format above. We allow several edges with equal "
    ...
    "Sometimes dialog can correspond to several iterations of loop, for example: "
...
check_graph_utterances_prompt = PromptTemplate.from_template(
    "You have a dialog and a structure of graph built on this dialog it is a "
    "set of nodes when chatbot system responses and a set of transitions that are triggered by user requests.\n"
    "Please say if for every utterance in the dialog there exist either a utteranse in node or in some edge. "
...
check_graph_validity_prompt = PromptTemplate.from_template(
    "1. You have an example of dialog from customer chatbot system.\n"
...
# After assistant's phrase first goes direct user's answer in any dialog, "medium please" in the example above.
...
# Further follows user's alternative answer then dialog flow returns to its standard way.
...
1. Dialog Flow Requirements:
   ...
   - Avoid any ambiguous or overly generic responses
   - Every dialog shall begin with some introduction like greeting etc
...
cycle_graph_generation_prompt_informal = PromptTemplate.from_template(
    """
Create a dialog graph for real conversations between two people about {topic}. The graph must follow these requirements:
...
1. Dialog Flow Requirements:
   ...
   - Every dialog shall begin with some introduction like greeting etc
   - Every dialog must be logically complete

2. Graph Structure Requirements:
   - Nodes are assistant's utterances, edges are utterances from the user
   - Dialog flow goes from source to target for all edges
...
Fix the invalid transitions in this dialog graph while keeping its structure.
...
Add extra edges to this dialog graph while keeping its structure.
...
part_1 = """Your input is a list of dialogs from customer chatbot system.
...
part_1i = """Your input is a dialog graph from customer chatbot system.
Your task is to extend set of nodes for the dialog graph with input dialog.
Dialog graph is a set of nodes with assistant's utterances and a set of edges that are
triggered by user requests. Here goes the input graph: """

part_1i0 = """Your input is a dialog graph from customer chatbot system.
Your task is to extend the dialog graph with input dialog.
Dialog graph is a set of nodes with assistant's utterances and a set of edges that are
triggered by user requests. Here goes the input graph: """
...
# e. Don't miss any assistant's utterance in all the dialogs.
...
# for example, one entity can be a particular case of the other in the dialog conext;
...
# If one entity is a particular case of the other in the dialog conext, they are considered synonyms.
...
# If speaker refers to something absent in the previous context, such dialogs
# 7) New dialogs which appear as a result of grouping process in step 5) shall be logical and following the context.
# If you see that groups cause such situations:
# new resulting dialogs look illogical,
# or speaker refers to a non-existent context,
# then such groups shall be ungrouped.

# 10) Example when dialog doesn't follow previous context:
...
# First utterance asks about price, while second one refers to price mentioned before, that means dialog flows are different and hence nodes are different.
# 7) To follow the context means to keep all the details from previous utterances of the dialog path in mind and follow them.
...
# integrate assistant's utterances from the dialog into existing graph nodes when you see it reasonable
# or add new nodes when it's not reasonable to intergate in existing nodes.
# Every assistant's utterance from the dialog either goes to existing node or creates new node.
...
# 11) Doublecheck that all the assistant's utterances from the dialog are present in resulting set of nodes,
...
# 8) For example if client was renting a car in London earlier in a dialog path, it can't be changed to renting a car in Long Beach later for no reason.
# So when you found illogical dialog path where London became Long Beach for unknown reason,
# place utterances with different locations into different nodes.
# client was renting a car in London earlier in a dialog path, it can't be changed to renting a car in Long Beach later for no reason.
# 9) You shall use every assistant's utterance from the dialog for one node only.
...
5) When a node contains more than one utterance, it means that any of these utterances can be part of dialog paths going thru this node,
so number of dialog paths is multiplied and all the utterances combinations of all the nodes in the path form the dialog paths.
...
6) As a result of combining nodes there can appear new dialog paths. And not all of them can be logical.
...
8) Consider all the dialog paths which appear as a result of combining utterances and make sure they are coherent and logical.
...
12) If user's utterance is the last in the dialog, try to find most suitable continuation node of a dialog flow
...
13) Doublecheck that all the utterances from the dialog are present in resulting graph,
...
I will give you the dialog, your task is to extend the graph above with the dialog according to the rules and examples above.
The dialog: """
...
If they aren't, separate your nodes when needed.
9) You mustn't remove assistant's utterances even if they are similar or synonymous. Make sure you keep all the assistant's utterances in the resulting set of nodes.
10) Doublecheck that all the assistant's utterances from the dialog are present in resulting set of nodes,
not a single assistant's utterance to be missed.
11) You must always return valid JSON fenced by a markdown code block. Do not return any additional text.
12) Add reason point to your answer with an explanation why you removed some of utterances.
I will give you the dialog, your task is to extend a set of nodes of the graph above with assistant's utterances from the dialog according to the rules and examples above.
The dialog: """
...
# 7) Combining nodes into groups must not allow dialog flow to be distorted.
# 8) New dialogs which appear as a result of grouping process shall be logical and following the context.
# 7) Every assistant's utterance not included in any group shall be present in its own group of single utterance.
# 8) You must doublecheck that all the assistant's utterances are present in resulting set of nodes, not a single utterance to be missed.
...
# 6) Dialog flow resulting the grouping process shall remain natural and conscious.
...
# 10) Make sure that nodes in dialog flow following combined node are coherent.

# 9) Make sure that combining assistant's utterances into nodes does not distort dialog flow logics and context.
# 10) Make sure that dialog flows don't have contradictory chains (according to point 4).
...
# 9) Make sure not a single dialog path has mutually exclusive essential concepts like cheap-expensive, or center-south, etc.
# 9) Make sure not a single dialog path has mutually exclusive essential concepts between any node and edges connected to it.
...
utterances mean that there are dialog flows connecting all the combinations of them (every utterance from each node with
...
you shall consider all its pairs with assistant's utterances of same intent in all the dialogs. If you can swap this pair members (along
with immediate contexts) between each other while sticking to the general meaning of the dialog,
...
Utterance 10a. asks about price, while utterance 10b. refers to price mentioned before, that means dialog flows are different and can't be part of one node.
11) Always place adjacent assistant's utterances into different nodes.
12) Consider all the dialog paths (according to point 4) which appear as a result of combining utterances and make sure they are coherent and following current context.
...
1) is_start field is mandatory for all the nodes.
2) is_start=True field in the node is an entry point to the whole graph.
3) Nodes must be assistant's utterances from the dialogs in their original form.
...
'Do you like Italian food? I know good restaurant in your price range.'
First utterance asks about price, while second one refers to price mentioned before, that means dialog flows are different and hence nodes are different.
9) Consider all the dialog paths which appear as a result of combining utterances and make sure they follow current context.
If they don't, modify your nodes.
10) Example when dialog doesn't follow current context:
User: I am looking for a cheap restaurant.
Assistant: I found 5 expensive restaurants for you.
11) You shall use every assistant's utterance from the list of the dialogs for one node only.
12) Doublecheck that all the assistant's utterances are present in resulting set of nodes, not a single utterance to be missed.
...
14) Don't modify dialog utterances even slightly (even a single letter) before placing them into the nodes.
15) You must always return valid JSON fenced by a markdown code block. Do not return any additional text.
16) Add reason point to your answer with an explanation why you didn't use all the assistant's utterances from all the dialogs.
I will give a list of dialogs, your task is to build a set of nodes containing all the utterances from this list according to the rules and examples above.
List of dialogs: """
...
# 10) Add reason point to the graph with your explanation why you didn't group assistant's utterances and why you didn't use all the assistant's utterances in dialogs.
# I will give a list of dialogs, your task is to build a set of nodes for this list according to the rules and examples above.
# List of dialogs: """
