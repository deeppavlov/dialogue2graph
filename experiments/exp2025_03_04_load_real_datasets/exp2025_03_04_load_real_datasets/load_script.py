...
def split2json(split: pd.DataFrame, domain: str) -> list[dict]:
    ...
    for idx, row in split.iterrows():
        ...
        if row["Message.From"] == "agent":
            ...
            cur.append(
                {"text": row["Message.Text"], "participant": row["Message.From"]}
            )
...
def take_e2e(path_to_files):
    ...
    files = [
        p
        for p in Path(path_to_files).iterdir()
        if p.is_file() and str(p).endswith(".tsv")
    ]
    json_data = []
    for file in files:
...
class d2gRealDialogs(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(
            "SMD",
            version=VERSION,
            description="This part of my dataset covers Stanford Multi-Domain Dataset",
        ),
        datasets.BuilderConfig(
            "MULTIWOZ2_2",
            version=VERSION,
            description="This part of my dataset covers MultiWOZ 2.2",
        ),
        datasets.BuilderConfig(
            "META_WOZ",
            version=VERSION,
            description="This part of my dataset covers MetaLWOz",
        ),
        datasets.BuilderConfig(
            "SCHEMA",
            version=VERSION,
            description="This part of my dataset covers The Schema-Guided Dialog dataset",
        ),
        datasets.BuilderConfig(
            "MSR-E2E",
            version=VERSION,
        ...
        datasets.BuilderConfig(
            "Frames",
            version=VERSION,
            description="This part of my dataset covers https://www.microsoft.com/en-us/research/project/frames-dataset/",
        ),
        datasets.BuilderConfig(
            "TaskMaster3",
            version=VERSION,
            description="This part of my dataset covers taskmaster3 movie ticketing dialogs",
        ),
        datasets.BuilderConfig(
            "WOZ",
            version=VERSION,
        ...
        if not my_dir.is_dir():
            ...
            if self.config.name == "SMD":
                urllib.request.urlretrieve(
                    "https://nlp.stanford.edu/projects/kvret/kvret_dataset_public.zip",
                    my_dir.joinpath("kvret_dataset_public.zip"),
                )
                with zipfile.ZipFile(
                    my_dir.joinpath("kvret_dataset_public.zip"), "r"
                ) as zip_ref:
                ...
                dataset = datasets.load_dataset(
                    ...
                    "MULTIWOZ2_2",
                    token=env_settings.HUGGINGFACE_TOKEN,
                ...
                dataset = datasets.load_dataset(
                    "microsoft/meta_woz", trust_remote_code=True
                )
                ...
                dataset = datasets.load_dataset(
                    "GEM/schema_guided_dialog", trust_remote_code=True
                )
                dataset["train"].to_json(my_dir.joinpath("train.jsonl"))
                dataset["validation"].to_json(my_dir.joinpath("dev.jsonl"))
                dataset["test"].to_json(my_dir.joinpath("test.jsonl"))
                ...
                dataset = datasets.load_dataset(
                    "google-research-datasets/taskmaster3", trust_remote_code=True
                )
                test_train_dataset = dataset["train"].train_test_split(test_size=0.1)
                valid_test_dataset = test_train_dataset["test"].train_test_split(
                    test_size=0.5
                )
                ...
                with zipfile.ZipFile(
                    my_dir.joinpath("Frames-dataset.zip"), "r"
                ) as zip_ref:
                    zip_ref.extractall(save_path)
                json_data = load_frames(
                    my_dir.joinpath("Frames-dataset").joinpath("frames.json")
                )
            ...
            for key, row in enumerate(f):
                ...
                if self.config.name == "SMD":
                    ...
                    exist = [
                        ...
                        {"text": u["data"]["utterance"], "participant": "user"}
                        if u["turn"] == "driver"
                        else {
                    ...
                    exist = [
                        [
                            {"text": u["user utterance"], "participant": "user"},
                            {"text": u["system response"], "participant": "assistant"},
                        ]
                        for u in data["log"]
                    ]
                    log = [
                    ...
                    exist = [
                        ...
                        for a, u in zip(data["turns"][0:-1:2], data["turns"][1::2])
                    ] + [[{"text": data["turns"][-1], "participant": "assistant"}]]
                    ...
                    exist = (
                        ...
                        + [
                            ...
                            for u, a in zip(
                                data["context"][0:-1:2], data["context"][1::2]
                            )
                        ]
                        + [
                            [
                                {"text": data["prompt"], "participant": "user"},
                    ...
                    exist = [
                        ...
                        {"text": u["text"], "participant": u["speaker"]}
                    ...
                    exist = [
                        ...
                        [
                            {"text": u["transcript"], "participant": "user"},
                            {
                                "text": a["system_transcript"],
                                "participant": "assistant",
                            },
                        ]
