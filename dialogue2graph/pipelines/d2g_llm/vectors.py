...
class DialogueStore:
    ...
    def _load_dialogue(self, dialogue: list, embeddings: HuggingFaceEmbeddings):
        ...
        assistant = [Document(page_content=d['text'].lower(), id=id, metadata={"id":id}) for id,d in enumerate([d for d in dialogue if d['participant']=='assistant'])]
        user = [Document(page_content=d['text'].lower(), id=id, metadata={"id":id}) for id,d in enumerate(d for d in dialogue if d['participant']=='user')]
    ...
    def search_assistant(self, utterance):
        docs = self.assistant_store.similarity_search_with_relevance_scores(utterance.lower(), k=self.assistant_size, score_threshold=env_settings.EMBEDDER_TYPO)
        # print("DOCS: ", docs)
        # print("UTT: ", utterance)
