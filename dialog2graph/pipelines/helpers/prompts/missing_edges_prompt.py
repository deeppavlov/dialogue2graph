add_edge_prompt_1 = """Your input is a dialog graph from customer chatbot system - it is
...
3) Dialog flow goes from source to target in every edge.
Please consider the graph above, list of dialogs below and do the following:
...
you shall find in the graph the most suitable continuation node of a dialog flow
...
9) So it is necessary to add new edges to the input graph from utterances which may exist in dialogs but absent in the graph.
...
12) Never create new or modify existing utterances from list of dialogs.
...
14) You must check all the dialogs and add this new edge to the node with assistant's utterance from current pair for all the dialog.
15) You must take not just all missing user's utterances but their pairs with previous assistant's utterances in all the dialogs.
...
I will give a list of dialogs, your return is a fixed version of dialog graph above according to the rules above.
