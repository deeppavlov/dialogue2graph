...
class TwoStagesGraphGenerator(GraphGenerator):
    prompt_name: str = ""
    ...
    def invoke(self, dialogue: list[Dialogue] = None, graph: DialogueGraph = None, topic: str = "") -> BaseGraph:
        ...
        return Graph(graph_dict=graph_dict)
        # partial_variables = {}
        # prompt_extra = ""
        # for idx, dial in enumerate(dialogue):
        #     #partial_variables[f"var_{idx}"] = dial.to_list()
        #     partial_variables[f"var_{idx}"] = dial.to_list()
        #     prompt_extra += f" Dialogue_{idx}: {{var_{idx}}}"
        # prompt = PromptTemplate(template=edges_1+"{graph_example_1}. "+edges_2+"{nodes}. "+edges_3+prompt_extra, input_variables=["graph_example_1","nodes"], partial_variables=partial_variables)
        ...
        # result = call_llm_api(prompt.format(graph_example_1=graph_example_1,nodes=nodes), model, temp=0)
        # # print("RESULT: ", result)
        # if result is None:
        #     return Graph(graph_dict={})
        # result.reason = reason + " Edges: " + result.reason
