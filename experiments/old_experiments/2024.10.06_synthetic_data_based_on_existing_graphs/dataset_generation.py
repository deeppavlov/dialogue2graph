...
with open(
    "/home/askatasuna/Документы/DeepPavlov/chatsky-llm-autoconfig/data/data.json"
) as f:
    graph_templates = {}
        ...
        while not success and counter <= retries:
            try:
                result = await model.ainvoke(
                    gen_prompt.format(
                        SCHEMA=graph_templates[graph_type]["empty"],
                        TARGET=graph_templates[graph_type]["target"],
                    )
                )
