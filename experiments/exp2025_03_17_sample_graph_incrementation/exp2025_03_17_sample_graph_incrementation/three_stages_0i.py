...
class ThreeStagesGraphGenerator(GraphGenerator):
    ...
    def invoke(self, dialogue: list[Dialogue] = None, graph: Graph = None, topic: str = "") -> BaseGraph:
        ...
        base_model = ChatOpenAI(model=env_settings.GENERATION_MODEL_NAME, api_key=env_settings.OPENAI_API_KEY, base_url=env_settings.OPENAI_BASE_URL)
        model = base_model | PydanticOutputParser(pydantic_object=DialogueGraph)
        graph_dict = call_llm_api(prompt.format(graph=graph.graph_dict), model).model_dump()
