...
#     "You have an example of a dialog from customer chatbot system. You also have an "
...
#     "I will give a dialog, your task is to build a graph for this dialog in the format above. We allow several edges with same "
...
#     "should be united in one node. Do not make up utterances that aren’t present in the dialog. Please do not combine "
#     "utterances from multiedges in one list, write them separately like in example above. Every utterance from the dialog, "
...
prompts["general_graph_generation_prompt"] = PromptTemplate.from_template(
    ...
    "I will give a dialog, your task is to build a cyclic graph for this dialog in the format above. "
    ...
    # "Every dialog corresponds to a cycled graph, for example: {dialog_example_2}. "
    ...
    "All the dialogs you've prompted are cyclic. "
    "Before answering you must check where the dialog can cycle and make the first node of a cycle a target node for the last node of the cycle. "
    # "Return ONLY JSON string in plain text (no code blocks) without any additional commentaries. "
    "You must always return valid JSON fenced by a markdown code block. Do not return any additional text. "
    "Here goes the dialog, build a cyclic graph according to the instructions above. "
    "Dialog: {dialog}"
)
...
prompts["general_graph_generation_prompt_try_2"] = PromptTemplate.from_template(
    ...
    "triggered by user requests. "
    "Here is the graph example: {graph_example_1}"
    "I will give a dialog, your task is to build a graph for this dialog in the format above. "
    # "We allow several edges with same "
    # "source and target and also multiple responses in one node so try not to add new nodes if it is logical just to extend an "
    # "exsiting one. Utterances in one node or in multiedge should be close to each other and correspond to different answers "
    # "to one question or different ways to say something. For example, for question about preferences or a Yes/No question "
    ...
    # "Do not forget ending nodes with goodbyes. "
    "Dialog can correspond to several iterations of loop, for example: {dialog_example_2}. "
    "It shall result in the graph below: {graph_example_2}. "
    "This is the end of the example. "
    ...
    "All the dialogs you've prompted are cyclic. "
    "Before answering you must check where the dialog can loop or cycle and make the first node of a cycle a target node for the last node of the cycle. "
    "Return ONLY JSON string in plain text (no code blocks) without any additional commentaries. "
    "Dialog: {dialog}"
)
...
#     "Nodes must be assistant's utterances, edges must be utterances from the user. "
#     # "Do not forget ending nodes with goodbyes. "
#     "Dialog may to contain cycles, for example: {dialog_example_2}"
#     "It shall result in the graph below: {graph_example_2}"
#     "This is the end of the example."
#     #"A cycle is a closed path in a graph, which means that it starts and ends at the same vertex and passes through a sequence of distinct vertices and edges."
...
#     # "Use cycle in a graph when you see that assistant's next answer logically is a phrase which already was used earlier in dialog, and make this node the first node of this cycle. "
...
#     "And you see it in a dialog: Next user's phrase is: Start new order, "
...
#     "IMPORTANT: All the dialogs you've prompted are cyclic. "
#     # "Before answering you must check where the dialog can loop or cycle and make the first node of a cycle a target node for the last node of the cycle. "
#     # "All the dialogs start from assistant's utterance, so first node of any loop cannot be first node of the whole graph. "
...
prompts["specific_graph_generation_prompt"] = PromptTemplate.from_template(
    "You have a dialog from customer chatbot system as input. "
    "Your task is to create a cyclic dialog graph corresponding to that dialog."
    ...
    # "Every transition in all edges shall start from and result in nodes which are present in set of nodes."
    "Another dialog example: {dialog_example_2}"
    "It shall result in the graph below: {graph_example_2}"
    "This is the end of the example."
    # "A cycle is a closed path in a graph, which means that it starts and ends at the same vertex and passes through a sequence of distinct vertices and edges."
    # "Use cycle in a graph when you see that assistant repeats phrase which already was used earlier in dialog, and make this repeat the first node of this cycle. "
    # "This repeat means that you don't create new node, but use node you created before for this assistant's utterance. "
    # "User's utterance in a dialog before this repeating utterance will be an edge leading from cycle's last node to the cycle's start node (the node with the repeating assistant's utterance). "
    "**Rules:**"
    "1) Nodes must be assistant's utterances, edges must be utterances from the user. "
    "2) Every utterance from the dialog, "
    "whether it is from user or assistanst, shall be present in the graph. "
    "3) Do not make up utterances that aren’t present in the dialog. "
    ...
    "I will give a dialog, your task is to build a graph for this dialog according to the rules and examples above. "
    # "Do not make up utterances that aren’t present in the dialog. "
    # "Please do not combine "
    # "utterances from multiedges in one list, write them separately like in example above. "
    # "Do not forget ending nodes with goodbyes. "
    # "IMPORTANT: All the dialogs you've prompted are cyclic so the conversation MUST return to an existing node"
    # "When you go to next user's utterance, first try to answer to that utterance with utterance from one of previously created nodes. "
    # "If you see it is possible not to create new node with same or similar utterance, but instead create next edge connecting back to that node, then it is place for a cycle here. "
    # "The cycle shall be organized so that you don't duplicate either user's utterances or nodes with same utterances. "
    # "Before answering you must check where the dialog can loop or cycle and make the first node of a cycle a target node for the last node of the cycle. "
...
prompts["second_graph_generation_prompt"] = PromptTemplate.from_template(
    "Your input is a dialog from customer chatbot system. "
    "Your task is to create a cyclic dialog graph corresponding to the dialog. "
    "Next is an example of the graph (set of rules) how chatbot system looks like - it is "
    ...
    "3) Every assistance's utterance from the dialog shall be present in one and only one node of a graph. "
    "4) Every user's utterance from the dialog shall be present in one and only one edge of a graph. "
    "5) Use ony utterances from the dialog. It is prohibited to create new utterances different from input ones. "
...
prompts["third_graph_generation_prompt"] = PromptTemplate.from_template(
    ...
    "**Rules:**"
    "1) Nodes must be assistant's utterances, edges must be utterances from the user. "
    "2) Every assistance's utterance from the dialog shall be present in one and only one node of a graph. "
    "3) Every user's utterance from the dialog shall be present in one and only one edge of a graph. "
    "4) Use ony utterances from the dialog. It is prohibited to create new utterances different from input ones. "
    "6) Never create nodes with user's utterances. "
    "7) Graph must be cyclic - no dead ends. "
...
prompts["fourth_graph_generation_prompt"] = PromptTemplate.from_template(
    ...
    # "10) Use one of assistant's utterances from the dialog for the cycle point, don't add/create more nodes with same or simiar utterances. "
    ...
    "10) Number of nodes and edges cannot exceed number of utterances in a dialog. "
...
compare_graphs_prompt = PromptTemplate.from_template(
    "You will get two dialog graphs in following format: {graph_example_1}. "
