...
#             DialogMessage(participant=line.split("\t")[0], text=line.split("\t")[1]) for line in string.strip().split("\n")
#         ]
#         return cls(messages=messages)

#     @classmethod
#     def from_list(cls, messages: List[Dict[str, str]], validate: bool = True) -> "Dialog":
#         """Create a Dialog from a list of dictionaries."""
#         dialog_messages = [DialogMessage(**m) for m in messages]
#         return cls(messages=dialog_messages, validate=validate)
...
#         """
#         new_messages = [msg if isinstance(msg, DialogMessage) else DialogMessage(**msg) for msg in messages]
...
class GraphGenerationResult(BaseModel):
    """Complete result with graph and dialogs"""
